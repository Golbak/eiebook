...
bool IsButtonHeld(ButtonNameType eButton_, u32 u32ButtonHeldTime_)
{
 ...
 if( (Button_asStatus[(u8)eButton_].eCurrentState == PRESSED) )
 {
   /* Check is button is held for a certain time */
   if( IsTimeUp(&Button_asStatus[eButton_].u32DebounceTimeStart, u32ButtonHeldTime_ ) )
   {
    return(TRUE);
   }
 }
...
void ButtonStartDebounce(u32 u32BitPosition_, PortOffsetType ePort_)
{
  ...
  if(eButton != NOBUTTON)
  {
    pu32InterruptAddress = (u32*)(&(AT91C_BASE_PIOA->PIO_IDR) + ePort_);
    *pu32InterruptAddress |= u32BitPosition_;
